...
class Proxy:
    ...
    def getNewProxyList(self, proxyCount, testURL):
        count = 0
        finish = False
        page = 1
        while(not(finish)):
            ...
            for proxy in proxyList:
                self.changeProxy({'http':'http://%s'%(proxy)})
                try:
                    res = urllib2.urlopen(testURL)
                    print 'ok: %s => %d'%(proxy, count+1)
...
|_| |_|_|\__,_|\___|_| |_| |_|\__, |\__,_|___/___/  Proxy Grabber
    ...
    parser.add_argument('-n', action='store', type=int, dest='numberOfProxies',
        help='Number of Proxies you want to grabbe' )
    parser.add_argument('-t', action='store', dest='testPage', help='URL to test for. Like http://google.com' )
 
    #args = parser.parse_args(['-f', 'proxy.txt', '-n', '2', '-t', 'twitter.com' ])
 
    args = parser.parse_args(sys.argv[1:])
    print banner
 
    if(args.fileName and args.numberOfProxies and args.testPage):
        if(not(re.match(r'^http://.*',args.testPage))):
            args.testPage = 'http://%s'%(args.testPage)
        proxy = Proxy()
        proxy.getNewProxyList(args.numberOfProxies, args.testPage)
